...
namespace Translator_desktop.LexicalAnalyzer
{
    static class Checker
    {
        ...
        {
            return OutputTokenTable.GetTable().Count != 0 && OutputTokenTable.GetTable().Last().Name == "=" //output tokens is exist and last token in table is "="
                && IdnTable.Contains(OutputTokenTable.GetTable()[OutputTokenTable.GetTable().Count - 2].Name) //penultimate token is identifier
                && IdnTable.Contains(token) //current token is identifier
                && (IdnTable.GetType(token) == IdnTable.GetType(OutputTokenTable.GetTable()[OutputTokenTable.GetTable().Count - 2].Name)); //types of tokens on both sides of "=" are equal
        }
    }
}